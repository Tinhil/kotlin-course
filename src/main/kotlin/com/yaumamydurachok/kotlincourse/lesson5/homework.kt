...
//
//((4 * 3) < 13) && !(8 / 4 == 2) || (10 % 3 != 1) //true && !true || false = false
//((14 - 5) > 8) || (9 / 3 == 3) && !(7 + 0 != 7) //true || true && !false = true
//((2 + 3) <= 5) && (16 / 4 == 4) || !(6 % 2 == 0) //true && true || !true = true
//((5 * 3) != 15) && !(10 / 2 > 4) || (4 + 4 == 8) //false && !true || true = true
...
//fun soundafterfall() {
//Тут я вообще не понял, как и чего делать. Пропорционально, все дела - сложно. Извините.

//}

fun fullPrice(clientPay: Double?) {
    val endPrice = clientPay ?: 50.0 + (clientPay ?: 50.0 / 200)
    println(endPrice)
}

fun meteo(pressure: String?) {
    val eggog: String = "Ошибка. Отсутствуют показания атмосферного давления"
    println(pressure ?: eggog)
}